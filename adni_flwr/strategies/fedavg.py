...
class FedAvgStrategy(FLStrategyBase):
    ...
    def configure_fit(self, server_round: int, parameters: Parameters, client_manager: ClientManager) -> List[Tuple[ClientProxy, FitIns]]:
        ...
        # Add server_round and scheduler information to the config for each client
        updated_instructions = []
        for client_proxy, fit_ins in client_instructions:
            # Add server_round and scheduler step info to the existing config
...
class FedAvgClient(ClientStrategyBase):
    ...
    def prepare_for_round(self, server_params: Parameters, round_config: Dict[str, Any]):
        ...
        if self.scheduler is not None and scheduler_step > 0:
            ...
            print(f"FedAvgClient: Scheduler fast-forwarded to step {scheduler_step}, LR: {current_lr_before:.8f} -> {current_lr_after:.8f}")
